...
function ListItem({ data: { title, description }, onDeleteClick }: Props) {
  return (
    <li className="flex items-center justify-between w-full px-4 py-2 space-x-2 text-white bg-blue-700 rounded-lg">
      <div className="space-y-1">
        <h3 className="font-bold">{title}</h3>
        <p className="text-sm italic">{description}</p>
      </div>
      <IconButton size="large" onClick={onDeleteClick}>
        <IconDisplay name={IconNames.Cross} className="stroke-white" />
      </IconButton>
    </li>
  );
}
